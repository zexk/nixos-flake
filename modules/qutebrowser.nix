_: {
  flake.homeModules.qutebrowser =
    { config, ... }:
    let
      qute-pass = "spawn --userscript qute-pass --dmenu-invocation 'dmenu -l 10' --password-store ${config.xdg.dataHome}/password-store";
    in
    {
      programs.qutebrowser = {
        ...
        keyBindings = {
          normal = {
            ...
            ",p" = qute-pass;
            ",P" = "${qute-pass} --password-only";
            ",u" = "${qute-pass} --username-only";
            ",o" = "${qute-pass} --otp-only";
            "J" = "tab-prev";
            "K" = "tab-next";
          };
          prompt = {
